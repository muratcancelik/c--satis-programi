...
namespace CosmoMillaCosmetics
{
    public partial class fSatis : Form
    {
                    ...
                    else
                    {
                        ...
                        if (db.Terazi.any(a => a.TeraziOnEk == onek))
                        {
                            ...
                            else
                            {
                                ...
                                MessageBox.Show("Kg Urun Ekleme Sayfasi");
                            }
        ...
        private void bH_MouseDown(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)
            {
                ...
                if (!b.Text.StartsWith("-"))
                {
                    ...
                    sil.Text = "Temizle - Buton NO:" + buttonid.ToString();//sag tikladigimizda bu 
                    sil.Click += Sil_Click;//sol tikladigimizda bunlari yapacak
                    s.Items.Add(sil);
        ...
        {
            //alttaki satir ust satirdaki sil.text komutundaki textteki karakterlerden sonra butonid almamiza yariyor("Temizle - Buton NO:")
        ...
        private void TlIsareti()
        {
            //numaratordeki sayilarin yanina tl isareti atiyor
        ...
        private void bBarkod_Click(object sender, EventArgs e)
        {
            if (tNumarator.Text != "")
            {
                ...
                {
                    MessageBox.Show("Urun ekleme sayfasini ac");
                }
        ...
        private void bDigerUrun_Click(object sender, EventArgs e)
        {
            if (tNumarator.Text != "")
            {
                ...
                gridSatisListesi.Rows[satirsayisi].Cells["UrunAdi"].Value = "Barkodsuz Urun";
                gridSatisListesi.Rows[satirsayisi].Cells["UrunGrup"].Value = "Barkodsuz Urun";
                ...
                gridSatisListesi.Rows[satirsayisi].Cells["KdvTutari"].Value = 01;
        ...
        public void SatisYap(string odemesekli)
        {
            ...
            if (satirsayisi > 0)
            {
                ...
                for (int i = 0; i < satirsayisi; i++)
                {
                    ...
                    satis.SatisFiyat = Islemler.DoubleYap(gridSatisListesi.Rows[i].Cells["AlisFiyati"].Value.ToString());
                    satis.Miktar = Islemler.DoubleYap(gridSatisListesi.Rows[i].Cells["Toplam"].Value.ToString());
                    ...
                    if (satisiade)
                    {
                        Islemler.StokAzalt(gridSatisListesi.Rows[i].Cells["Barkod"].Value.ToString(), Islemler.DoubleYap(gridSatisListesi.Rows[i].Cells["Toplam"].Value.ToString()));
                    }
                ...
                {
                    io.Aciklama = "Iade islemi(" + odemesekli + ")";
                ...
                db.SaveChanges();
                MessageBox.Show("Yazdirma Islemi Yap");
                Temizle();
        ...
        private void bNakitbtn_Click(object sender, EventArgs e)
        {
            SatisYap("Nakit");

        }

        private void bKartbtn_Click(object sender, EventArgs e)
        {
            SatisYap("Kart");
        }

        private void bKartNakitbtn_Click(object sender, EventArgs e)
        {
        ...
        {
            //f12 basinca nakit odeme
            if (e.KeyCode == Keys.F12)
            {
                SatisYap("Nakit");
            }
            //f2 basinca kart odeme 
            if (e.KeyCode == Keys.F2)
                SatisYap("Kart");

            //f3 basinca nakitkart kismi aciliyo
        ...
        {
            if(bIslemBeklet.Text=="Islem Beklet")
            {
                Bekle();
                bIslemBeklet.BackColor = System.Drawing.Color.OrangeRed;
                bIslemBeklet.Text = "Islem Bekliyor";
                gridSatisListesi.Rows.Clear();
            }
            else
            {
                ...
                bIslemBeklet.Text = "Islem Beklet";
